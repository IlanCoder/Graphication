...
void Application::update(){
	time += 0.000005f;
	//oPlane.angleX = oPlane.angleX+0.0005f;
	oPlane.angleY = oPlane.angleY + 0.05f;
}

void Application::setup(){
	oPlane.createPlane(100);
	std::string strVertexShader = loadTextFile("Shaders/passThru.v");
	std::string strFragmentShader = loadTextFile("Shaders/funkShader.f");
	oPlane.texture_id[0] = initTexture4("BumpWall.png");
	oPlane.texture_id[1] = initTexture3("Wall.png");
	...
	uMyLightPosition[0] = glGetUniformLocation(oPlane.shader[0], "myLightPosition");
...
void Application::display(){
	...
	glBindVertexArray(oPlane.vao);
	rotationX = glm::rotate(glm::mat4(1.0f), (oPlane.angleX * 3.141592f / 180.0f), glm::vec3(1.0f, 0.0f, 0.0f));
	rotationY = glm::rotate(glm::mat4(1.0f), (oPlane.angleY * 3.141592f / 180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	rotationZ = glm::rotate(glm::mat4(1.0f), (oPlane.angleX * 3.141592f / 180.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	rotation = rotationX * rotationY * rotationZ;
	lookAt = glm::lookAt(eye, target, up);
	perspective = glm::perspective(45.0f, 1920.0f / 1080.0f, 0.1f, 200.0f);
	transform = perspective*lookAt*rotation;
...
GLuint Application::initTexture3(const std::string name) {
...
GLuint Application::initTexture4(const std::string name){
	int width, height, channels;
	unsigned char* img = SOIL_load_image(name.c_str(), &width,&height,&channels,0);
	GLuint tex = 0;
	glGenTextures(1, &tex);
